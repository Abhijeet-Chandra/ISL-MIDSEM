...
# - Use Buyer, Supplier and Auditor roles and route each order to one supplier.
# - Replace DES with AES-CTR and store the generated nonce with ciphertext.
# - Add order ID and PENDING/ACCEPTED/REJECTED status with decision time.
# - The Auditor sees order metadata and verifies signatures without decrypting.
# =====================================================================
# ================= EXACT ALGORITHMS USED =================
# Encryption/decryption: AES-128 in CTR mode; nonce stored before ciphertext.
...
AES-CTR, SHA-256 and RSA-signature role system for procurement orders.
"""
...
def decrypt_data(encrypted_data, key=CIPHER_KEY):
    ...
    return AES.new(key, AES.MODE_CTR, nonce=nonce).decrypt(ciphertext).decode()
