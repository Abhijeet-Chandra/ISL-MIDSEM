...
# - Read actual file bytes and encrypt them with a fresh random AES session key.
# - Encrypt the AES key with the Receiver's RSA public key (RSA-AES hybrid).
# - Filter downloads by receiver and sign/hash the encrypted file bytes.
# - Administrator sees metadata/audit information and cannot decrypt files.
# =====================================================================
...
RSA-OAEP protects a random AES session key; AES-EAX protects data; RSA/SHA-256 authenticates uploads.
"""
...
# ========================== FIXED SETTINGS ==========================
DATA_FILE = "cloudsafe_records.pkl"
RECORDS = []
USER_KEYS = {}
...
def aes_decrypt(package, aes_key):
    ...
    return cipher.decrypt_and_verify(package["data"], package["tag"])
...
def get_input_data():
    ...
    choice = input("Choice: ")
    if choice == "1":
        return input("Enter data: ").encode()
    if choice == "2":
...
def upload_record(username):
    ...
    route_value = input("Receiver username: ")
    encrypted = encrypt_data(data)
    public_key, private_key = get_user_keys(username)
    RECORDS.append({
        "owner": username,
        "receiver": route_value,
        "encrypted": encrypted,
        "encrypted_hash": hash_data(encrypted["data"]),
        "plain_hash": hash_data(data),
        "signature": sign_data(encrypted["data"], private_key),
...
def review_record(username):
    allowed = [record for record in RECORDS if record.get("receiver") == username]
    record = select_record(allowed)
    if record is None:
        return

    public_key = RSA.import_key(record["public_key"])
    signature_ok = verify_signature(record["encrypted"]["data"], record["signature"], public_key)
    encrypted_hash_ok = hash_data(record["encrypted"]["data"]) == record["encrypted_hash"]
