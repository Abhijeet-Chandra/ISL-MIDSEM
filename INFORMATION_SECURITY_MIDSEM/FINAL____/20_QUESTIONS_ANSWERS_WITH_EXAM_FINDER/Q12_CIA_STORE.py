...
# - Use Document Owner, Authorized User and Auditor roles.
# - Provide confidentiality with random AES plus RSA-encrypted session keys.
# - Provide integrity with SHA-256 and authenticity with RSA signatures.
# - Filter by assigned recipient and immediately deny revoked users.
# =====================================================================
...
RSA-AES hybrid confidentiality, SHA-256 integrity, RSA authenticity and three-role access control.
"""
...
# ========================== FIXED SETTINGS ==========================
DATA_FILE = "ciastore_records.pkl"
RECORDS = []
USER_KEYS = {}
REVOKED_USERS = set()
...
def upload_record(username):
    ...
    route_value = input("Authorized user: ")
    encrypted = encrypt_data(data)
    public_key, private_key = get_user_keys(username)
    RECORDS.append({
        "owner": username,
        "recipient": route_value,
        "encrypted": encrypted,
...
def review_record(username):
    allowed = [record for record in RECORDS if record.get("recipient") == username]
