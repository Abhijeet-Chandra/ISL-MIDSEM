...
# - Encryption/decryption: AES-128 in OFB mode.
# - Record hashing: SHA-256.
# - Digital signature: 2048-bit RSA with SHA-256.
# - Variant filename: ROLE_AES_OFB.py
# =====================================================================
...
"""Ready-to-run role template tailored specifically for AES OFB."""
...
DATA_FILE = "q07_cloud_safe_aes_ofb_sha256_records.pkl"
RECORDS = []
USER_KEYS = {}
...
def encrypt_data(data, key=CIPHER_KEY):
    return AES.new(key, AES.MODE_OFB, iv=IV).encrypt(data.encode())


def decrypt_data(encrypted_data, key=CIPHER_KEY):
    return AES.new(key, AES.MODE_OFB, iv=IV).decrypt(encrypted_data).decode()
