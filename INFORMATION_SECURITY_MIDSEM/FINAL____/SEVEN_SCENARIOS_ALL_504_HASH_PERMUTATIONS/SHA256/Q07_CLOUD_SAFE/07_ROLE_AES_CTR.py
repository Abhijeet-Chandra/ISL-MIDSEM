...
# - Uploader selects a receiver and uploads text from keyboard or a text file.
# - Receiver opens only files addressed to that receiver and verifies integrity.
# - Administrator sees owners, receivers, hashes and timestamps, not plaintext.
# - The confidentiality algorithm is replaced by the named permutation below.
#
# EXACT CRYPTOGRAPHY IN THIS SOLUTION
# - Encryption/decryption: AES-128 in CTR mode.
# - Record hashing: SHA-256.
# - Digital signature: 2048-bit RSA with SHA-256.
# - Variant filename: ROLE_AES_CTR.py
# =====================================================================
...
"""Ready-to-run role template tailored specifically for AES CTR."""
...
NONCE = b"12345678"
DATA_FILE = "q07_cloud_safe_aes_ctr_sha256_records.pkl"
RECORDS = []
USER_KEYS = {}
...
def encrypt_data(data, key=CIPHER_KEY):
    return AES.new(key, AES.MODE_CTR, nonce=NONCE).encrypt(data.encode())


def decrypt_data(encrypted_data, key=CIPHER_KEY):
    return AES.new(key, AES.MODE_CTR, nonce=NONCE).decrypt(encrypted_data).decode()
