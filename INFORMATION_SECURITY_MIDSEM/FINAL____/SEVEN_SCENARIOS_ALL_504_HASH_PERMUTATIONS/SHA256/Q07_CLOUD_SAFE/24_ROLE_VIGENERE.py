...
# - Encryption/decryption: Vigenere cipher (key SECURITY).
# - Record hashing: SHA-256.
# - Digital signature: 2048-bit RSA with SHA-256.
# - Variant filename: ROLE_VIGENERE.py
# =====================================================================
...
"""Ready-to-run role template tailored specifically for VIGENERE."""
...
# ========================== FIXED SETTINGS ==========================
CIPHER_KEY = "SECURITY"
DATA_FILE = "q07_cloud_safe_vigenere_sha256_records.pkl"
RECORDS = []
USER_KEYS = {}
...
def decrypt_data(encrypted_data, key=CIPHER_KEY):
    data, key = clean(encrypted_data), clean(key)
    return "".join(chr((ord(ch) - 65 - (ord(key[i % len(key)]) - 65)) % 26 + 65)
                   for i, ch in enumerate(data))
