...
# Encryption: DES/AES/3DES selected in EDIT BOX 2, using ECB mode.
...
DATA_FILE = "secure_records.pkl"  # Change only if a filename is given.
RECORDS = []
USER_KEYS = {}
...
def decrypt_data(encrypted_data, key=CIPHER_KEY):
    algorithm = cipher_module()
    cipher = algorithm.new(key, algorithm.MODE_ECB)
    return unpad(cipher.decrypt(encrypted_data), algorithm.block_size).decode()
