...
# Record hashing: custom 32-bit, MD5, SHA-1 or SHA-256 (EDIT BOX 2).
# Digital signature: 2048-bit RSA with SHA-256.

"""ROLE TEMPLATE: custom hash, MD5, SHA-1 or SHA-256."""
...
# ============================ EDIT BOX 2 ============================
# Change HASH_ALGORITHM only: "custom", "md5", "sha1" or "sha256".
HASH_ALGORITHM = "sha256"
CIPHER_KEY = b"0123456789ABCDEF"       # Auxiliary AES-128 encryption.

DATA_FILE = "hash_records.pkl"  # Change only if a filename is given.
RECORDS = []
USER_KEYS = {}
...
def hash_data(data):
    ...
    if HASH_ALGORITHM == "custom":
        value = 5381
        for byte in data:
            value = ((value * 33 + byte) ^ (value >> 13)) & 0xFFFFFFFF
        return hex(value)
