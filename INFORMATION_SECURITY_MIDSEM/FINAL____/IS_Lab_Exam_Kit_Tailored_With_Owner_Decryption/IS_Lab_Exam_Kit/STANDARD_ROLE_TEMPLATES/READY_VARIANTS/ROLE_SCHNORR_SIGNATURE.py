...
# Primary digital-signature algorithm: Schnorr signature with SHA-256.
# Auxiliary encryption: AES-128 ECB.
# Record hashing: SHA-256.
# =========================================================

"""Ready-to-run role template tailored specifically for SCHNORR SIGNATURE."""
...
DATA_FILE = "role_schnorr_signature_records.pkl"
RECORDS = []
USER_KEYS = {}
...
def sign_data(data, private_key):
    ...
    r = pow(g, k, p)
    e = int.from_bytes(SHA256.new(data_bytes(data) + str(r).encode()).digest(), "big") % q
...
def verify_signature(data, signature, public_key):
    ...
    expected = int.from_bytes(SHA256.new(data_bytes(data) + str(r).encode()).digest(), "big") % q
    return e == expected
